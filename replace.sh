...
const facultyContent = \`import facultyData from '../solution-weeks-faculty-coverage.json';
...
fs.writeFileSync('src/lib/facultyMatcher.ts', facultyContent);

const smeContent = \`import smeData from '../../solution-weeks-sme-coverage.json';
import { getKnownUtcOffset } from './timezones';
...
fs.writeFileSync('src/lib/smeMatcher.ts', smeContent);
"
